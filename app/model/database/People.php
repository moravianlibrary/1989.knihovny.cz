...
class People {
    ...
    function get_rand_people_from_group($group, $limit = 8){
        $res = $this->db->query('SELECT `name`, `gender`, `born`, `dead`, `job`, `desc`, `wrote`, `theywrote`, COALESCE(b.path, "files/img/osoby/undefined.png") as `img` FROM `person` a JOIN upload b ON b.ID=a.img WHERE', [
            'a.cat' => $group
